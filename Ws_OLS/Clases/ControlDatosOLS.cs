...
namespace Ws_OLS.Clases
{
	public class ControlDatosOLS
	{
        ...
        public void CambiaEstadoFCCCF(int ruta, string FC, string fecha, string numero)
		{
		...
		public void CambiaEstadoFANU(int ruta, string FC, string fecha, string numero)
		{
			...
			using (SqlConnection cnn = new SqlConnection(connectionString))
			{
				...
				//                                WHERE idRuta=@ruta AND CAST(FechaIngreso AS DATE)=@fecha AND idTipoSerie=@idTipo ";
				string sqlQuery = @"UPDATE HandHeld.FacturaEBajada
									SET FELAutorizacion=1, FeLAnulacionNumero=1
									WHERE idRuta=@ruta AND CAST(Fecha AS DATE)=@fecha AND TipoDocumento=@FC AND Numero=@numero";
		...
		public void CambiaEstadoNC(int ruta, string fecha, string numero)
		{
		...
		public void CambiaEstadoNR(int ruta, string fecha, string corr)
		{
